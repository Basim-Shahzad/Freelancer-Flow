...
import { daysOfWeek, months, getLastSevenDates, getLastDays, getLast12Months } from "../helpers.js";
...
const DashboardRevenueSection = () => {
   ...
   const { data: paidInvoices } = useInvoices(false, "paid");
   ...
   const { data, trend } = useMemo(() => {
      ...
      (paidInvoices?.invoices ?? []).forEach((invoice) => {
         ...
         if (timeRange === "monthly" || timeRange === "all") {
            ...
            key = daysOfWeek[d.getUTCDay()] ?? undefined; // right now only checks that day not in the last 7 days
         }

         if (key && key in buckets) {
            buckets[key]! += Number(invoice.total);
         }
      });
