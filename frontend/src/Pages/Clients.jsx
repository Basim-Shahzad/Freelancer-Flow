      ...
      [
         {
            ...
            enableSorting: false,
            header: null,
            cell: () => <div className='flex justify-center items-center' >
               <Checkbox color="secondary" className='border-2 border-white' sx={{
                  color: 'white',
         ...
         {
            accessorKey: 'name',
            header: <div className='flex items-center gap-1' ><CiUser />Name </div>,
            cell: (props) => <div>{props.getValue()}</div>
         },
         {
            accessorKey: 'email',
            header: <div className='flex items-center gap-1' ><CiUser />Email</div>,
            cell: (props) => <div>{props.getValue()}</div>
         },
         {
            accessorKey: 'projects',
            header: <div className='flex items-center gap-1' ><CiUser />Projects</div>,
            cell: (props) => {
               const project = props.getValue()[0]
               const projectLenghtExc = props.getValue().length - 1
               return project ? <div>{props.getValue()[0]} {projectLenghtExc != 0 ? <span> and {projectLenghtExc} more </span> : ''} </div> : <div>No Projects</div>
            }
         },
         {
            accessorKey: 'active',
            header: <div className='flex items-center gap-1' ><CiUser />Active</div>,
            cell: (props) => {
               const isActive = props.getValue();
               return (
                  <div
                     className={`px-2 py-1 w-max text-sm ${isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                  >
      ...
      <div className='bg-[#081028] h-screen flex flex-col px-2.5 items-center overflow-hidden'>
         <div className="top-bar xl:w-full py-4 sm:py-8 grid grid-cols-1 xl:grid-cols-2 gap-3 items-center">
            ...
            <div className='xl:flex xl:w-full xl:justify-end'>
               <button className='bg-[rgb(203,60,255)] hover:bg-[rgb(175,60,255)] transition-colors duration-100 cursor-pointer text-[16px] text-white px-8 py-1 xl:mr-7 w-19/20 sm:py-2 rounded-md xl:w-1/3 xl:h-max' >Add Client</button>
            </div>
